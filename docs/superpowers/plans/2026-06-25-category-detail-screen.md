...
- `app.css` is a committed, prebuilt Tailwind artifact. Any new utility class used in HTML/JS only takes visual effect after `npm run build:css`; render tests assert on class strings and do not need the rebuild.
...
- `public/css/app.css` — rebuilt (Task 5).
...
Regenerate the prebuilt `app.css` so the new utility classes (`block`, `hover:border-sage`, `transition-colors`, `gap-x-4`, `gap-y-1`, `py-4`, etc.) are present, then run the automated gates and a manual smoke check.

**Files:**
- Modify: `public/css/app.css` (generated)

- [ ] **Step 1: Rebuild the CSS**

Run: `npm run build:css`
Expected: completes without error; `public/css/app.css` is regenerated (it will now contain the new utilities scanned from `dashboard.js`, `category.js`, and `category.html`).
...
- [ ] **Step 4: Commit the rebuilt CSS**

```bash
git add public/css/app.css
git commit -m "style: rebuild Tailwind CSS for category screen and dashboard links" -m "Co-Authored-By: Claude Opus 4.8 <noreply@anthropic.com>"
```
