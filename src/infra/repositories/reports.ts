...
type DashboardCategoryRow = Category & { group_name: string; group_color: string; group_sort: number };
...
export function makeReportRepository(db: Db): ReportRepository {
  return {
    ...
    spendByGroupMonth(groupId: number, month: string): number {
      ...
      ).get(groupId, month) as { s: number }).s;
    },
