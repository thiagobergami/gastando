...
export interface CategoryUseCaseDeps { categories: CategoryRepository; groups: GroupRepository; }

export interface CreateCategoryInput { group_id: number; name: string; examples?: string; sort_order?: number; }
export interface UpdateCategoryInput {
  group_id: number; name: string; examples?: string; sort_order?: number; active?: number;
}
  ...
  return {
    ...
    create(input: CreateCategoryInput): Category {
      ...
      return categories.insert({
        group_id: input.group_id, name: input.name, examples: input.examples ?? '', sort_order,
      });
    },
    update(id: number, input: UpdateCategoryInput): Category {
      ...
      const changes = categories.update(id, {
        group_id: input.group_id, name: input.name, examples: input.examples ?? '',
        sort_order: input.sort_order ?? 0, active,
      });
